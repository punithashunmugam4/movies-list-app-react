...
const Seats=()=>{
    ...
    return(
        ...
        <div className="allSeats"> 
            ...
            {
             section1.map((seat)=>{
                return <div key={'key'+seat} className={availableSeats.includes(seat) ? "filled":"available"} id={seat} onClick={(e)=>handleSeat(e)}>{seat}</div>
             })   
            }
            </div>
           ...
           { selectedSeat.length>0 && <Link to='/customer-details' state={[selectedSeat,bookedMovie]}><button className="booking-btn">Book Seats</button></Link> }
        </div>
    )
}
